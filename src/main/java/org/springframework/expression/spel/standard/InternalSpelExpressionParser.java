...
class InternalSpelExpressionParser extends TemplateAwareExpressionParser {
	...
	// The org.springframework.org.springframework.expression being parsed
	private String expressionString = "";

	// The token stream constructed from that org.springframework.org.springframework.expression string
	private List<org.springframework.expression.spel.standard.Token> tokenStream = Collections.emptyList();
	...
	protected SpelExpression doParseExpression(String expressionString, @Nullable ParserContext context)
		...
		try {
			...
			org.springframework.expression.spel.standard.Tokenizer tokenizer = new org.springframework.expression.spel.standard.Tokenizer(expressionString);
			...
			org.springframework.expression.spel.standard.Token t = peekToken();
			if (t != null) {
	...
	//	    | (QMARK^ org.springframework.org.springframework.expression COLON! org.springframework.org.springframework.expression)
	//      | (ELVIS^ org.springframework.org.springframework.expression))?;
	@Nullable
	...
	private SpelNodeImpl eatLogicalOrExpression() {
		...
		while (peekIdentifierToken("or") || peekToken(TokenKind.SYMBOLIC_OR)) {
			org.springframework.expression.spel.standard.Token t = takeToken();  //consume OR
	...
	private SpelNodeImpl eatLogicalAndExpression() {
		...
		while (peekIdentifierToken("and") || peekToken(TokenKind.SYMBOLIC_AND)) {
			org.springframework.expression.spel.standard.Token t = takeToken();  // consume 'AND'
	...
	private SpelNodeImpl eatRelationalExpression() {
		...
		org.springframework.expression.spel.standard.Token relationalOperatorToken = maybeEatRelationalOperator();
		if (relationalOperatorToken != null) {
			org.springframework.expression.spel.standard.Token t = takeToken();  // consume relational operator token
			...
			if (tk == TokenKind.IN) {
				return new OperatorIn(t.startPos, t.endPos, expr, rhExpr);
			}
	...
	private SpelNodeImpl eatSumExpression() {
		...
		while (peekToken(TokenKind.PLUS, TokenKind.MINUS, TokenKind.INC)) {
			org.springframework.expression.spel.standard.Token t = takeToken();  //consume PLUS or MINUS or INC
	...
	private SpelNodeImpl eatProductExpression() {
		...
		while (peekToken(TokenKind.STAR, TokenKind.DIV, TokenKind.MOD)) {
			org.springframework.expression.spel.standard.Token t = takeToken();  // consume STAR/DIV/MOD
	...
	private SpelNodeImpl eatPowerIncDecExpression() {
		...
		if (peekToken(TokenKind.POWER)) {
			org.springframework.expression.spel.standard.Token t = takeToken();  //consume POWER
			SpelNodeImpl rhExpr = eatUnaryExpression();
			checkRightOperand(t, rhExpr);
			return new OperatorPower(t.startPos, t.endPos, expr, rhExpr);
		}
		if (expr != null && peekToken(TokenKind.INC, TokenKind.DEC)) {
			org.springframework.expression.spel.standard.Token t = takeToken();  //consume INC/DEC
	...
	private SpelNodeImpl eatUnaryExpression() {
		if (peekToken(TokenKind.PLUS, TokenKind.MINUS, TokenKind.NOT)) {
			org.springframework.expression.spel.standard.Token t = takeToken();
			SpelNodeImpl expr = eatUnaryExpression();
	...
	private SpelNodeImpl eatDottedNode() {
		org.springframework.expression.spel.standard.Token t = takeToken();  // it was a '.' or a '?.'
	...
	private boolean maybeEatFunctionOrVar() {
		...
		org.springframework.expression.spel.standard.Token t = takeToken();
		org.springframework.expression.spel.standard.Token functionOrVariableName = eatToken(TokenKind.IDENTIFIER);
	...
	private void consumeArguments(List<SpelNodeImpl> accumulatedArguments) {
		...
		int pos = t.startPos;
		org.springframework.expression.spel.standard.Token next;
		do {
	...
	private int positionOf(@Nullable org.springframework.expression.spel.standard.Token t) {
		if (t == null) {
			// if null assume the problem is because the right token was
			// not found at the end of the org.springframework.org.springframework.expression
	...
	private boolean maybeEatBeanReference() {
		if (peekToken(TokenKind.BEAN_REF) || peekToken(TokenKind.FACTORY_BEAN_REF)) {
			org.springframework.expression.spel.standard.Token beanRefToken = takeToken();
			org.springframework.expression.spel.standard.Token beanNameToken = null;
	...
	private boolean maybeEatTypeReference() {
		if (peekToken(TokenKind.IDENTIFIER)) {
			org.springframework.expression.spel.standard.Token typeName = peekToken();
	...
	private boolean maybeEatNullReference() {
		if (peekToken(TokenKind.IDENTIFIER)) {
			org.springframework.expression.spel.standard.Token nullToken = peekToken();
	...
	//projection: PROJECT^ org.springframework.org.springframework.expression RCURLY!;
	...
	private boolean maybeEatInlineListOrMap() {
		...
		org.springframework.expression.spel.standard.Token closingCurly = peekToken();
	...
	private SpelNodeImpl eatPossiblyQualifiedId() {
		...
		org.springframework.expression.spel.standard.Token node = peekToken();
	...
	private boolean isValidQualifiedId(@Nullable org.springframework.expression.spel.standard.Token node) {
	...
	private boolean maybeEatMethodOrProperty(boolean nullSafeNavigation) {
		if (peekToken(TokenKind.IDENTIFIER)) {
			org.springframework.expression.spel.standard.Token methodOrPropertyName = takeToken();
			SpelNodeImpl[] args = maybeEatMethodArgs();
			if (args == null) {
				// property
	...
	private boolean maybeEatConstructorReference() {
		if (peekIdentifierToken("new")) {
			org.springframework.expression.spel.standard.Token newToken = takeToken();
	...
	//parenExpr : LPAREN! org.springframework.org.springframework.expression RPAREN!;
	...
	private org.springframework.expression.spel.standard.Token maybeEatRelationalOperator() {
		...
		if (t.isIdentifier()) {
			...
			if (idString.equalsIgnoreCase("in")) {
				return t.asInToken();
			}
		}
		return null;
	...
	private org.springframework.expression.spel.standard.Token eatToken(TokenKind expectedKind) {
		org.springframework.expression.spel.standard.Token t = nextToken();
		if (t == null) {
	...
	private org.springframework.expression.spel.standard.Token takeToken() {
	...
	public String toString(@Nullable org.springframework.expression.spel.standard.Token t) {
		if (t == null) {
			return "";
		}
	...
	private void checkOperands(org.springframework.expression.spel.standard.Token token, @Nullable SpelNodeImpl left, @Nullable SpelNodeImpl right) {
		checkLeftOperand(token, left);
		checkRightOperand(token, right);
	}

	private void checkLeftOperand(org.springframework.expression.spel.standard.Token token, @Nullable SpelNodeImpl operandExpression) {
		if (operandExpression == null) {
			throw internalException(token.startPos, SpelMessage.LEFT_OPERAND_PROBLEM);
		}
	}

	private void checkRightOperand(org.springframework.expression.spel.standard.Token token, @Nullable SpelNodeImpl operandExpression) {
		if (operandExpression == null) {
