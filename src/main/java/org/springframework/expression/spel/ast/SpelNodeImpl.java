 ...
 * format org.springframework.org.springframework.expression.
 *
 * @author Andy Clement
...
public abstract class SpelNodeImpl implements SpelNode, Opcodes {
	...
	private static final org.springframework.expression.spel.ast.SpelNodeImpl[] NO_CHILDREN = new org.springframework.expression.spel.ast.SpelNodeImpl[0];
	...
	protected org.springframework.expression.spel.ast.SpelNodeImpl[] children = NO_CHILDREN;

	@Nullable
	private org.springframework.expression.spel.ast.SpelNodeImpl parent;

	/**
	 * Indicates the type descriptor for the result of this org.springframework.org.springframework.expression node.
	...
	public SpelNodeImpl(int startPos, int endPos, org.springframework.expression.spel.ast.SpelNodeImpl... operands) {
		this.startPos = startPos;
		this.endPos = endPos;
		if (!ObjectUtils.isEmpty(operands)) {
			this.children = operands;
			for (org.springframework.expression.spel.ast.SpelNodeImpl operand : operands) {
	...
	protected boolean nextChildIs(Class<?>... classes) {
		if (this.parent != null) {
			org.springframework.expression.spel.ast.SpelNodeImpl[] peers = this.parent.children;
	 ...
	 * the current org.springframework.org.springframework.expression being compiled is available in the codeflow object, e.g.
	 ...
	 * @param arguments the org.springframework.org.springframework.expression nodes for the org.springframework.org.springframework.expression supplied argument values
	 */
	protected static void generateCodeForArguments(MethodVisitor mv, CodeFlow cf, Member member, org.springframework.expression.spel.ast.SpelNodeImpl[] arguments) {
		...
		if (isVarargs) {
			...
			org.springframework.expression.spel.ast.SpelNodeImpl lastChild = (childCount == 0 ? null : arguments[childCount - 1]);
			...
			if (lastChild != null && arrayType.equals(lastChild.getExitDescriptor())) {
				...
				int arrayindex = 0;
				while (p < childCount) {
					org.springframework.expression.spel.ast.SpelNodeImpl child = arguments[p];
					mv.visitInsn(DUP);
	 ...
	 */
	protected static void generateCodeForArgument(MethodVisitor mv, CodeFlow cf, org.springframework.expression.spel.ast.SpelNodeImpl argument, String paramDesc) {
