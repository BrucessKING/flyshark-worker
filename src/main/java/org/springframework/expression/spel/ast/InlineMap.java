...
/**
 * Represent a map in an org.springframework.org.springframework.expression, e.g. '{name:'foo',age:12}'
 *
 * @author Andy Clement
 * @author Sam Brannen
 * @since 4.1
	...
	private void checkIfConstant() {
		...
		for (int c = 0, max = getChildCount(); c < max; c++) {
			...
			if (!(child instanceof Literal)) {
				...
				else if (child instanceof org.springframework.expression.spel.ast.InlineMap inlineMap) {
		...
		if (isConstant) {
			...
			for (int c = 0; c < childCount; c++) {
				...
				else if (valueChild instanceof org.springframework.expression.spel.ast.InlineMap inlineMap) {
