...
/**
 * An org.springframework.org.springframework.expression parser that understands templates. It can be subclassed by org.springframework.org.springframework.expression
...
public abstract class TemplateAwareExpressionParser implements ExpressionParser {
	...
	/**
	 * Helper that parses given org.springframework.org.springframework.expression string using the configured parser. The
	 * org.springframework.org.springframework.expression string can contain any number of expressions all contained in "${...}"
	 ...
	 * 'foo${abc}'. This means that org.springframework.org.springframework.expression languages that used ${..} as part of their
	 * functionality are supported without any problem. The parsing is aware of the
	 * structure of an embedded org.springframework.org.springframework.expression. It assumes that parentheses '(', square
	 * brackets '[' and curly brackets '}' must be in pairs within the org.springframework.org.springframework.expression unless
	 * they are within a string literal and a string literal starts and terminates with a
	 * single quote '.
	 * @param expressionString the org.springframework.org.springframework.expression string
	...
	private Expression[] parseExpressions(String expressionString, ParserContext context) throws ParseException {
		...
		while (startIdx < expressionString.length()) {
			...
			if (prefixIndex >= startIdx) {
				// an inner org.springframework.org.springframework.expression was found - this is a composite
				if (prefixIndex > startIdx) {
				...
				if (suffixIndex == -1) {
					throw new ParseException(expressionString, prefixIndex,
							"No ending suffix '" + suffix + "' for org.springframework.org.springframework.expression starting at character " +
							prefixIndex + ": " + expressionString.substring(prefixIndex));
				}
				if (suffixIndex == afterPrefixIndex) {
					throw new ParseException(expressionString, prefixIndex,
							"No org.springframework.org.springframework.expression defined within delimiter '" + prefix + suffix +
	 ...
	 * supplied org.springframework.org.springframework.expression string.
	 * @param expressionString the org.springframework.org.springframework.expression string which may contain the suffix
	...
	private int skipToCorrectEndSuffix(String suffix, String expressionString, int afterPrefixIndex)
		...
		// Chew on the org.springframework.org.springframework.expression text - relying on the rules:
	...
	/**
	 * Actually parse the org.springframework.org.springframework.expression string and return an Expression object.
	 ...
	 * org.springframework.org.springframework.expression. The positional information is used if an error has to be reported
